...
const pitchColors = {
  ...
  ST: '#008080', CU: '#800080', KC: '#4B0082', CH: '#008000', FS: '#4682B4',
  FO: '#B22222', CS: '#9370DB', KN: '#FFFF00', EP: '#A0522D', SV: '#20B2AA'
};
...
async function init() {
  ...
  teamSelect.addEventListener('change', () => {
    ...
    const pitchers = Object.keys(pitchData[selectedTeam]);
  ...
  pitcherSelect.addEventListener('change', () => {
    ...
    const pitchTypes = pitchData[selectedTeam][selectedPitcher];
    checkboxContainer.innerHTML = '';
    activeTypes.clear();
    balls.forEach(ball => scene.remove(ball));
    balls = [];

    for (const pitchType in pitchTypes) {
      ...
      checkbox.addEventListener('change', () => {
        if (checkbox.checked) {
          ...
          addBall(pitchTypes[pitchType], pitchType);
        } else {
