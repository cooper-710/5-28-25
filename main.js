...
function populateDropdowns(data) {
  ...
  pitcherSelect.addEventListener('change', () => {
    ...
    const pitchKeys = Object.keys(data[team][pitcher]);
  ...
  comboSelect.addEventListener('change', () => {
    const team = teamSelect.value;
    const pitcher = pitcherSelect.value;
    const selectedPitch = comboSelect.value;

    if (data[team][pitcher][selectedPitch]) {
      clearBalls();
...
function addBall(pitch, pitchType) {
  ...
  const t0 = clock.getElapsedTime();
    ...
    const selectedZone = document.getElementById('endZoneSelect').value;
  let targetOverride = null;
  if (selectedZone && zoneLocations[selectedZone]) {
    targetOverride = {
      x: zoneLocations[selectedZone].x,
      y: zoneLocations[selectedZone].y,
      z: -60.5
    };
  }
  ball.userData.targetOverride = targetOverride;
  console.log('Assigning targetOverride:', targetOverride);
  ball.userData = {
    type: pitchType,
    t0: t0,
    release: {
...
function populateDropdowns(data) {
  ...
  pitcherSelect.addEventListener('change', () => {
    ...
    const comboSelect = document.getElementById('comboSelect');
    comboSelect.innerHTML = '';
    const types = Object.keys(data[team][pitcher]);
    types.forEach(pitchKey => {
      const option = document.createElement('option');
      option.value = pitchKey;
      option.textContent = pitchKey;
      comboSelect.appendChild(option);
    });
