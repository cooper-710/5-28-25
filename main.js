...
function clearBalls() {
  ...
  document.getElementById('pitchCheckboxes').innerHTML = '';
    ...
    clearBtn.textContent = 'Clear All Pitches';
    clearBtn.addEventListener('click', () => {
      clearBalls();
      document.querySelectorAll('#pitchCheckboxes input[type=checkbox]').forEach(cb => cb.checked = false);
    });
...
function addCheckboxes(pitcherData) {
  ...
  for (const pitchType in pitchGroups) {
    ...
    titleWrapper.style.marginTop = '0px';
    ...
    masterCheckbox.style.transform = 'scale(1)';
    masterCheckbox.addEventListener('change', () => {
      for (let z = 1; z <= 9; z++) {
        const id = `${pitchType} ${z}`;
    ...
    titleWrapper.appendChild(masterCheckbox);
...
// Insert clear button once on load
