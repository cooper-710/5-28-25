...
function populateDropdowns(data) {
  comboSelect = document.getElementById('pitchTypeZoneSelect');
  ...
  teamSelect.addEventListener('change', () => {
    ...
    pitcherSelect.dispatchEvent(new Event('change'));
      ...
      comboSelect.innerHTML = '<option disabled selected>Select Combo</option>';
      const uniqueCombos = new Set();
      for (const pitchType in pitcherData) {
        for (const zone in Object.keys(pitcherData[pitchType])) {
          uniqueCombos.add(`${pitchType} ${zone}`);
        }
      }
      for (const combo of [...uniqueCombos].sort()) {
        const option = document.createElement('option');
        option.value = combo;
      ...
      comboSelect.addEventListener('change', () => {
        const [pitchType, zone] = comboSelect.value.split(' ');
        if (pitcherData[pitchType] && pitcherData[pitchType][zone]) {
          throwBall(pitcherData[pitchType][zone]);
        }
      });
